...
export default function WatchPage() {
	...
	useEffect(() => {
		if (beast && replayEvents.length > 0) {
			...
			const replayIndex = replayEvents.findIndex(
				(event) =>
					event.type === "beast" &&
			...
			if (replayIndex !== -1) {
				const adventurerIndex = replayEvents.slice(replayIndex).findIndex((event) => event.type === "adventurer");
	...
	const subscribeEvents = async (gameId: number) => {
		...
		if (!gameState) {
			enqueueSnackbar("Failed to load game", {
				variant: "warning",
				...
				callback: ({ data, error }: { data?: any[]; error?: Error }) => {
					if (data && data.length > 0) {
						const events = data
							...
							.filter((entity: any) => getEntityModel(entity, "GameEvent")?.adventurer_id === gameId)
