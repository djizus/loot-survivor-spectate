...
import { collectableImage, getBeastImageById, getCollectableTraits } from "@/utils/beast";
...
export default function BeastScreen({ setIsBattling }: BeastScreenProps) {
	...
	const [, setMenuAnchor] = useState<null | HTMLElement>(null);
	...
	const strike = useLottie({
		...
		onComplete: () => {
			...
			if (pendingBeastAttack.current) {
				...
				beastStrike.play();
				setCombatLog(
					`${beast!.baseName} attacked your ${pending.location} for ${pending.damage} damage ${pending.critical_hit ? "CRITICAL HIT!" : ""}`,
				);
			}
		},
	});
	...
	useEffect(() => {
		if (battleEvent && !skipCombat) {
			if (battleEvent.type === "attack") {
				...
				if (!fastBattle) {
					setCombatLog(
						`You attacked ${beast!.baseName} for ${battleEvent.attack?.damage} damage ${battleEvent.attack?.critical_hit ? "CRITICAL HIT!" : ""}`,
					);
				}
				...
				if (isPlayerStrikePlaying.current) {
					...
					beastStrike.play();
					setCombatLog(
						`${beast!.baseName} attacked your ${battleEvent.attack?.location} for ${battleEvent.attack?.damage} damage ${battleEvent.attack?.critical_hit ? "CRITICAL HIT!" : ""}`,
					);
				}
				...
				beastStrike.play();
				setCombatLog(
					`${beast!.baseName} ambushed your ${battleEvent.attack?.location} for ${battleEvent.attack?.damage} damage ${battleEvent.attack?.critical_hit ? "CRITICAL HIT!" : ""}`,
				);
			}
		}
	}, [battleEvent]);
	...
	return (
		<motion.div initial="initial" animate="animate" exit="exit" variants={screenVariants} style={styles.container}>
				...
				<Box sx={styles.topSection}>
					<Box sx={styles.beastInfo}>
						...
						<Box sx={styles.healthContainer} mt={2} mb={1}>
							...
							<LinearProgress variant="determinate" value={(beastHealth / beast!.health) * 100} sx={styles.healthBar} />
					...
					<Box sx={styles.beastImageContainer}>
						{collectable && isJackpot && (
							<Box sx={styles.toastContainer}>
								<Typography sx={styles.wantedBeastText}>WANTED BEAST</Typography>
							</Box>
						)}
						...
						{ownerName && (
							<Box sx={styles.collectableContainer}>
								<Typography sx={styles.ownerNameText}>Owned by {ownerName}</Typography>
							</Box>
						)}
					</Box>
				</Box>
				...
				<Box sx={styles.middleSection}>
					<Box sx={styles.combatLogContainer}>
						...
						{(combatLog === fleeMessage || combatLog === attackMessage || combatLog === equipMessage) && (
							<div className="dotLoader green" style={{ marginTop: "6px" }} />
						)}
					</Box>
				</Box>
				...
				<Box sx={styles.bottomSection}>
					<Box sx={styles.adventurerStatusRow}>
						<Box sx={styles.adventurerImageContainerSmall}>
							<img src={"/images/mobile/adventurer.png"} alt="Adventurer" style={styles.adventurerImageSmall} />
							{beastStrike.View}
						</Box>
						...
						<Box sx={styles.adventurerVitals}>
							<Box sx={styles.healthContainer}>
								...
								<LinearProgress variant="determinate" value={(health / maxHealth) * 100} sx={styles.healthBar} />
				...
				<Box sx={styles.equippedItemsContainer}>
					<Box sx={styles.equippedItemsGrid}>
						{Object.entries(slotIcons).map(([slot, icon]) => {
							...
							const isNameMatch = ItemUtils.isNameMatch(
								equippedItem!.id,
								level,
								adventurer!.item_specials_seed,
								beast!,
						...
						<Box sx={styles.itemDetailsContent}>
							...
							<Box sx={styles.itemInfoSection}>
								{selectedItem ? (
										...
										<Box sx={styles.itemStats}>
											{ItemUtils.isWeapon(selectedItem.id) && (
												<>
													...
													<Box sx={styles.compactInfoLine}>
														<Typography sx={styles.infoValue}>
															Deals {calculateAttackDamage(selectedItem, adventurer!, beast).criticalDamage} damage
															(critical)
														</Typography>
													</Box>
												</>
											...
											{!ItemUtils.isWeapon(selectedItem.id) &&
												!ItemUtils.isRing(selectedItem.id) &&
												!ItemUtils.isNecklace(selectedItem.id) && (
													<>
														<Box sx={styles.compactInfoLine}>
															<Typography sx={styles.infoValue}>
														...
														<Box sx={styles.compactInfoLine}>
															<Typography sx={styles.infoValue}>
																-{calculateBeastDamage(beast!, adventurer!, selectedItem).criticalDamage} Health
																(Critical)
															</Typography>
														</Box>
													</>
												...
												return (
													actualStatBonus && (
														<Box sx={styles.compactInfoLine}>
															...
															<Typography sx={styles.levelValue}>{actualStatBonus}</Typography>
														</Box>
													)
												);
											})()}
									...
									<Box>
										<Typography sx={styles.emptySlotText}>No item equipped</Typography>

										{/* Show damage for empty armor slots */}
										{selectedSlot &&
											["Head", "Chest", "Foot", "Hand", "Waist"].includes(selectedSlot) &&
											beast &&
											beast.health > 4 && (
												<Box sx={styles.itemStats}>
													<Box sx={styles.compactInfoLine}>
							...
							<Box sx={styles.availableItemsSection}>
								<Typography sx={styles.availableItemsTitle}>Swap item:</Typography>
																			...
																			>
																				<Typography
																					sx={[
																						styles.damageIndicatorText,
																						isArmorSlot
																							? styles.damageIndicatorTextRed
																							: styles.damageIndicatorTextGreen,
																					]}
																				>
							...
							<>
								<Box sx={styles.distributionRow}>
									...
									<Box sx={[styles.distributionCard, styles.positiveCard]}>
										...
										<Typography sx={styles.distributionValue}>{formatPercent(simulationResult.winRate)}</Typography>
									</Box>
									<Box sx={[styles.distributionCard, styles.warningCard]}>
										<Typography sx={styles.distributionLabel}>OTK Chance</Typography>
										<Typography sx={styles.distributionValue}>{formatPercent(simulationResult.otkRate)}</Typography>
									</Box>
								</Box>
								...
								<Box sx={styles.distributionRow}>
									<Box sx={styles.distributionCard}>
										...
										<Typography sx={styles.distributionValue}>{formatNumber(simulationResult.modeRounds)}</Typography>
									</Box>
									<Box sx={[styles.distributionCard, styles.positiveCard]}>
										<Typography sx={styles.distributionLabel}>Avg DMG Dealt</Typography>
										<Typography sx={styles.distributionValue}>
											{formatNumber(simulationResult.modeDamageDealt)}
										</Typography>
									</Box>
									<Box sx={[styles.distributionCard, styles.warningCard]}>
										<Typography sx={styles.distributionLabel}>Avg DMG Taken</Typography>
										<Typography sx={styles.distributionValue}>
