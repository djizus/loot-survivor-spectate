...
export default function AdventurerStats() {
	...
	const handleStatIncrement = (stat: keyof typeof STAT_DESCRIPTIONS) => {
		if (
			pointsRemaining > 0 &&
			selectedStats[stat] + adventurer!.stats[stat] < MAX_STAT_VALUE + equippedItemStats[stat]
		) {
			setSelectedStats({
				...selectedStats,
	...
	function STAT_HELPER_TEXT(stat: string, currentValue: number) {
		...
		if (stat === "strength") {
			...
			return advancedMode ? (
				`Potions: ${potionPrice(level, currentValue)} gold`
			) : (
				<Box>
			...
			<Box sx={styles.statGrid}>
				{["strength", "dexterity", "vitality", "intelligence", "wisdom", "charisma"].map((stat) => {
					const totalStatValue =
						adventurer?.stats?.[stat as keyof typeof STAT_DESCRIPTIONS]! +
						selectedStats[stat as keyof typeof STAT_DESCRIPTIONS]!;
					...
					return (
						<Box sx={styles.statRow} key={stat}>
							<Box sx={styles.statInfo}>
								...
								{effectText && <Typography sx={styles.statEffect}>{effectText}</Typography>}
							</Box>
							<Box sx={styles.statControls}>
								{adventurer?.stat_upgrades_available! > 0 && stat !== "luck" && (
									<Button
										variant="contained"
										size="small"
								...
								<Typography sx={styles.statValue}>
									...
									>
										{totalStatValue}
										{showItemStats &&
											!hasPendingStatUpgrades &&
											equippedItemStats[stat as keyof typeof STAT_DESCRIPTIONS] > 0 && (
								...
								{adventurer?.stat_upgrades_available! > 0 && stat !== "luck" && (
									<Button
										...
										disabled={
											adventurer!.stats[stat as keyof typeof STAT_DESCRIPTIONS] +
												selectedStats[stat as keyof typeof STAT_DESCRIPTIONS] >=
											MAX_STAT_VALUE + equippedItemStats[stat as keyof typeof STAT_DESCRIPTIONS]
										}
										sx={styles.controlButton}
									>
										+
	...
	const renderStatsView = () => (
		<>
			{Object.entries(STAT_DESCRIPTIONS)
				.filter(([stat]) => (adventurer!.stat_upgrades_available! > 0 ? stat !== "luck" : true))
								...
								title={
									<Box sx={styles.tooltipContainer}>
										<Box sx={styles.tooltipTypeRow}>
											<Typography sx={styles.tooltipTypeText}>{STAT_TITLE(stat)}</Typography>
										...
										<Box sx={styles.tooltipSection}>
											<Typography sx={styles.tooltipDescription}>{description}</Typography>
