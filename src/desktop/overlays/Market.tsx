...
import { type DamageBucket, type SlotDamageSummary, getExplorationInsights } from "@/utils/exploration";
import { calculateLevel } from "@/utils/game";
import { ItemType, ItemUtils, Tier, slotIcons, typeIcons } from "@/utils/loot";
import {
	type ArmorSetStatSummary,
	STAT_FILTER_OPTIONS,
	type StatDisplayName,
	generateMarketItems,
	getTierOneArmorSetStats,
	potionPrice,
} from "@/utils/market";
import { keyframes } from "@emotion/react";
import FilterListAltIcon from "@mui/icons-material/FilterListAlt";
import KeyboardDoubleArrowUpIcon from "@mui/icons-material/KeyboardDoubleArrowUp";
import { Box, IconButton, Modal, Tab, Tabs, ToggleButton, ToggleButtonGroup, Tooltip, Typography } from "@mui/material";
...
const isGearTab = (tab: SetStatsTab): tab is GearTab => tab === "Weapons" || tab === "Rings";

export default function MarketOverlay() {
	const { adventurer, bag, marketItemIds, exploreLog, gameSettings } = useGameStore();
	const {
		slotFilter,
		typeFilter,
		tierFilter,
	...
	const activeArmorSummary = isArmorTab(activeSetStatsTab) ? (armorSummariesByType[activeSetStatsTab] ?? null) : null;
	...
	const handleExplorationTabChange = useCallback(
		(_: SyntheticEvent, newValue: "overall" | "slot" | "discovery") => {
			if (newValue) {
	...
	const handleAmbushSlotChange = useCallback(
		(_: SyntheticEvent, newValue: SlotDamageSummary["slot"] | null) => {
			if (newValue) {
	...
	const handleTrapSlotChange = useCallback(
		(_: SyntheticEvent, newValue: SlotDamageSummary["slot"] | null) => {
			if (newValue) {
	...
	const renderDistributionList = useCallback((distribution: DamageBucket[], highlightValue: number | null = null) => {
		if (!distribution.length) {
			return <Typography sx={styles.distributionEmpty}>Not enough data yet.</Typography>;
		}
		...
		return (
			<Box sx={styles.distributionList}>
				{distribution.map((bucket) => {
					const isHighlighted =
						highlightValue !== null && highlightValue >= bucket.start && highlightValue <= bucket.end;
	...
	const explorationSection = useMemo(() => {
			...
			return (
				<Box sx={styles.exploringContent}>
					<Box sx={styles.exploringCard}>
						...
						<Typography sx={styles.placeholderMessage}>
							Encounter data will appear once the game state is loaded.
						</Typography>
					</Box>
				</Box>
			);
		...
		const ambushOverallLethal =
			hasPlayerHealth && beasts.damageDistribution.length > 0 ? beasts.overallLethalChance : null;
		const trapOverallLethal =
			hasPlayerHealth && obstacles.damageDistribution.length > 0 ? obstacles.overallLethalChance : null;
			...
			<>
				<Box sx={styles.exploringCard}>
					...
					<Typography sx={styles.lethalChance}>
						{ambushOverallLethal !== null ? `Lethal: ${ambushOverallLethal.toFixed(1)}% chance` : "Lethal: --"}
					</Typography>
				...
				<Box sx={styles.exploringCard}>
					...
					<Typography sx={styles.lethalChance}>
						{trapOverallLethal !== null ? `Lethal: ${trapOverallLethal.toFixed(1)}% chance` : "Lethal: --"}
					</Typography>
			...
			<>
				<Box sx={styles.exploringCard}>
					...
					<Box sx={styles.slotSection}>
						...
						{selectedAmbushSlot ? (
							renderDistributionList(selectedAmbushSlot.distribution, playerHealth)
						) : (
				...
				<Box sx={styles.exploringCard}>
					...
					<Box sx={styles.slotSection}>
						...
						{selectedTrapSlot ? (
							renderDistributionList(selectedTrapSlot.distribution, playerHealth)
						) : (
							<Typography sx={styles.distributionEmpty}>Not enough data yet.</Typography>
						)}
					</Box>
			...
			<Box sx={styles.exploringCard}>
				...
				<Box sx={styles.discoveryRow}>
					<Box sx={styles.discoveryCard}>
						...
						<Typography sx={styles.discoveryValue}>
							+{discoveries.goldRange.min} to +{discoveries.goldRange.max}
						</Typography>
					</Box>
					<Box sx={styles.discoveryCard}>
						<Typography sx={styles.discoveryLabel}>Health ({discoveries.healthChance}%)</Typography>
						<Typography sx={styles.discoveryValue}>
							+{discoveries.healthRange.min} to +{discoveries.healthRange.max}
						</Typography>
					</Box>
					<Box sx={styles.discoveryCard}>
						...
						exploreLog.map((event, index) => (
							<Box key={`${exploreLog.length - index}`} sx={styles.eventItem}>
								<Box sx={styles.eventIcon}>
									<Box component="img" src={getEventIcon(event)} alt={"event"} sx={styles.eventIconImage} />
								</Box>
								<Box sx={styles.eventDetails}>
									...
									<Box sx={styles.eventMeta}>
										...
										{event.items && event.items.length > 0 && (
											<Typography sx={styles.eventMetaValue}>{event.items.length} items</Typography>
										)}
	...
	const renderSetStatsItem = (item: ArmorSetStatSummary["items"][number]) => {
		...
		const statText = item.statBonus ? item.statBonus.replace(/\s+/g, " ").toUpperCase() : "—";

		return (
			<Box key={`${item.slot}-${item.id}`} sx={styles.setStatsItemRow}>
				{hasItem && imageUrl ? (
					<Box sx={[styles.setStatsItemImageContainer]}>
						<Box sx={[styles.setStatsItemGlow, tierColor ? { backgroundColor: tierColor } : {}]} />
						<Box component="img" src={imageUrl} alt={item.slot} sx={styles.setStatsItemImage} />
					</Box>
				) : (
	...
	const remainingGold = adventurer?.gold || 0;
			...
			<Box sx={[styles.popup, advancedMode && styles.advancedPopup]}>
				...
				{advancedMode && (
					<Box sx={styles.tabBar}>
						<Tabs value={activeTab} onChange={handleTabChange} aria-label="market sections" sx={styles.tabs}>
						...
						{marketAvailable && (
							<Box sx={styles.topBar}>
								<Box sx={styles.goldDisplay}>
									<Typography sx={styles.goldLabel} variant="h6">
										Gold left:
									</Typography>
									<Typography sx={styles.goldValue} variant="h6">
										{remainingGold}
									</Typography>
								</Box>
							</Box>
						...
						<Box sx={styles.mainContent}>
							...
							<Box sx={styles.itemsGrid}>
								{filteredItems.map((item) => {
									...
									const shouldGrayOut = (!canAfford && !isOwned) || isOwned;
									return (
										<Box key={item.id} sx={[styles.itemCard, shouldGrayOut && styles.itemUnaffordable]}>
											<Box sx={styles.itemImageContainer}>
												<Box sx={[styles.itemGlow, { backgroundColor: ItemUtils.getTierColor(item.tier) }]} />
												<Box
													component="img"
													src={item.imageUrl}
													alt={item.name}
												...
												<Tooltip
													...
													title={
														<Box sx={styles.tierTooltipContainer}>
															<Typography sx={styles.tierTooltipTitle}>Tier {item.tier}</Typography>
															...
															<Box sx={styles.tierTooltipDivider} />
															<Typography sx={styles.tierTooltipHint}>Lower tier = stronger item</Typography>
														</Box>
													}
												>
												...
												<Tooltip
													...
													title={
														<Box sx={styles.slotTooltipContainer}>
															...
															<Typography sx={styles.slotTooltipText}>Equips to your {item.slot}</Typography>
														</Box>
													}
												>
											...
											<Box sx={styles.itemInfo}>
												<Box sx={styles.itemHeader}>
													<Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
														...
														{item.type !== "Ring" && item.type !== "Necklace" && <JewelryTooltip itemId={item.id} />}
													</Box>
													<Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
														...
														<Typography sx={styles.itemType}>{item.type}</Typography>
													</Box>
												</Box>
												{adventurer?.item_specials_seed !== 0 &&
													(() => {
														...
														return statBonus ? (
															<Box sx={styles.statBonusBox}>
																<Typography sx={styles.itemStatBonus}>{statBonus}</Typography>
															</Box>
														) : null;
													})()}

												<Box sx={styles.itemFooter}>
													<Typography sx={styles.itemPrice}>{item.price} Gold</Typography>
												</Box>
											</Box>
										</Box>
									);
			...
			>
				<Box sx={styles.setStatsModal}>
					<Box sx={styles.setStatsHeader}>
						...
						<IconButton onClick={() => setShowSetStats(false)} sx={styles.setStatsCloseButton}>
							&times;
						</IconButton>
					</Box>

					{setStatsSummaries.length === 0 ? (
						<Typography sx={styles.setStatsEmpty}>No item data available</Typography>
					) : (
								...
								<Box sx={styles.setStatsColumn}>
									{weaponSummary && weaponSummary.items.length > 0 && (
										<>
											...
											<Box sx={styles.setStatsColumnItems}>{weaponSummary.items.map(renderSetStatsItem)}</Box>
										</>
									)}
									{ringSummary && ringSummary.items.length > 0 && (
										<>
											<Typography
												sx={[
													styles.setStatsColumnHeader,
													weaponSummary?.items.length ? { borderTop: "1px solid rgba(215, 198, 41, 0.25)" } : {},
												]}
											>
												RINGS
											</Typography>
											<Box sx={styles.setStatsColumnItems}>{ringSummary.items.map(renderSetStatsItem)}</Box>
										</>
									)}
								</Box>
							)}
