...
export default function ExploreOverlay() {
	...
	const { ambushLethalChance, trapLethalChance } = useExplorationWorker(adventurer ?? null, gameSettings ?? null);
	...
	const handleCheckout = () => {
		...
		const itemPurchases = cart.items.map((item) => {
			...
			const shouldEquip =
				(slotEmpty && !slotsToEquip.has(slot)) ||
				(slot === "weapon" &&
					[Tier.T1, Tier.T2].includes(ItemUtils.getItemTier(item.id)) &&
	...
	return (
		<Box sx={[styles.container, spectating && styles.spectating]}>
								...
								{event.type === "discovery" && event.discovery?.type && (
									<>
										...
										{event.discovery.type === "Health" && (
											<Typography sx={styles.encounterHeal}>+{event.discovery.amount} Health</Typography>
										)}
									</>
								)}
			...
			{/* Bottom Buttons */}
			{!spectating && (
				<Box sx={[styles.buttonContainer, advancedMode && styles.advancedButtonContainer]}>
					{advancedMode && (
					...
					{adventurer?.stat_upgrades_available! > 0 ? (
						<Button
							...
							sx={{
								...
								...(Object.values(selectedStats).reduce((a, b) => a + b, 0) === adventurer?.stat_upgrades_available &&
									styles.selectStatsButtonHighlighted),
							}}
			...
			{collectable && collectableTokenURI && (
				<BeastCollectedPopup onClose={() => setCollectable(null)} tokenURI={collectableTokenURI} beast={collectable} />
			)}
		</Box>
	);
}
